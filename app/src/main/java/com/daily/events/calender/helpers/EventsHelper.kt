...
class EventsHelper(val context: Context) {
    ...
    fun getEventsSync(
        ...
        var events = if (applyTypeFilter) {
            ...
            if (eventId == -1L) {
                eventsDB.getOneTimeEventsFromTo(toTS, fromTS).toMutableList()
            } else {
                eventsDB.getOneTimeEventFromToWithId(eventId, toTS, fromTS).toMutableList()
            }
        }
    ...
    fun getAnniversariesEventTypeId(createIfNotExists: Boolean = true): Long {
        ...
        if (eventTypeId == -1L && createIfNotExists) {
            val eventType = EventType(
                null,
                anniversaries,
                context.resources.getColor(R.color.default_anniversaries_color)
            )
            eventTypeId = insertOrUpdateEventTypeSync(eventType)
        }
        return eventTypeId
    ...
    fun getRepeatableEventsFor(
        ...
        val events = if (applyTypeFilter) {
            ...
            if (eventId == -1L) {
                eventsDB.getRepeatableEventsFromToWithTypes(toTS).toMutableList()
            } else {
                eventsDB.getRepeatableEventFromToWithId(eventId, toTS).toMutableList()
            }
        }
