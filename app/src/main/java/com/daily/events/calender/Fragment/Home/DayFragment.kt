 ...
 */
class DayFragment : Fragment(), WeeklyCalendar {
    ...
    override fun onCreateView(
        ...
        scrollView.onGlobalLayout {
            ...
            val initialScrollY = (rowHeight * 1).toInt()
        ...
        return ScaleGestureDetector(
            ...
            object : ScaleGestureDetector.SimpleOnScaleGestureListener() {
                override fun onScale(detector: ScaleGestureDetector): Boolean {
                    ...
                    var newFactor =
                        Math.max(Math.min(wantedFactor, MAX_SCALE_FACTOR), MIN_SCALE_FACTOR)
    ...
    private fun addAllDayEvent(event: Event) {
        (inflater.inflate(R.layout.week_all_day_event_marker, null, false) as TextView).apply {
            ...
            val dayOfWeek =
                dayColumns.indexOfFirst { it.tag.toInt() == dayCodeStart || (it.tag.toInt() > dayCodeStart && it.tag.toInt() <= dayCodeEnd) }
            if (dayOfWeek == -1) {
                return
            }
