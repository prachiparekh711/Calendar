...
class EventListWidgetAdapter(val context: Context) : RemoteViewsService.RemoteViewsFactory {
    ...
    private fun setupListEvent(remoteView: RemoteViews, item: ListEvent) {
        ...
        remoteView.apply {
            ...
            setText(
                R.id.event_item_description,
                if (replaceDescription) item.location else item.description
            )
            setText(
                R.id.event_item_start,
    ...
    private fun getItemViewType(position: Int) =
        if (events.getOrNull(position) is ListEvent) ITEM_EVENT else ITEM_HEADER
